...
const valid={name:'Test Attendee',address:'Test Street',city:'Colombo',contact:'077 123 4567',guests:'2',attendance:'Yes',registrationId:'CAMY-TEST-12345678'};
...
test('rejects non-POST and invalid data without writing',async()=>{
 ...
 try{
  ...
  for(const body of [null,'bad JSON',{...valid,name:' '},{...valid,contact:'-------'},{...valid,guests:'21'},{...valid,guests:'1.5'},{...valid,attendance:'No'},{...valid,registrationId:'=bad'}])assert.equal((await run(body)).code,400);
 }finally{global.fetch=original;}
});
test('sends normalized data and returns confirmed reference',async()=>{
 const original=global.fetch;
 global.fetch=async(url,options)=>{
  assert.equal(options.redirect,'follow');assert.equal(options.body.get('totalAttendees'),'3');assert.equal(options.body.get('name'),'Test Attendee');
